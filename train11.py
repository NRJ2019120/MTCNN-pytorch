...
class Trainer(nn.Module):   #nn.Module 所有模型的父类
    ...
    def train(self):
        ...
        #训练
        while True:
           for epoch,(imgdata, con, offset,landmarks) in enumerate(self.dataloader):  # batchsize
            if self.isCuda == True:              #GPU 运算
                imgdata, con, offset,landmarks = imgdata.cuda(), con.cuda(), offset.cuda(),landmarks.cuda()

            con_out, offset_out,landmarks_out = self.net(imgdata)   # imgdata为网络输入, 得到网络输出
